...
export function cancelProcess() {
    ...
    notifications.info("Cancelling processing...");
}
...
export async function process() {
    try {
        ...
        $("#outputSection").fadeIn(300);
        
        notifications.info("Starting image processing...");
        ...
        notifications.success("Image processed successfully!");
        ...
        log("Error: " + errorMessage + (errorStack ? " at " + errorStack : ""));
        
        if (errorMessage === "Cancelled") {
            notifications.warning("Processing was cancelled");
        } else {
            notifications.error("Error processing image: " + errorMessage);
        }
    } finally {
...
export async function updateOutput() {
    ...
    if (processResult != null) {
        ...
        $("#statusSVGGeneratePercent").text("0%");
        ...
        const svg = await GUIProcessManager.createSVG(processResult.facetResult, processResult.colorsByIndex, sizeMultiplier, fill, stroke, showLabels, fontSize, fontColor, (progress) => {
            ...
            $("#statusSVGGenerate").css("width", progressPercent + "%");
            $("#statusSVGGeneratePercent").text(progressPercent + "%");
        });
        
        $("#statusSVGGenerate").css("width", "100%");
        $("#statusSVGGeneratePercent").text("100%");
...
export function downloadPalettePng() {
    if (processResult == null) {
        notifications.warning("Please process an image first");
        return;
    }
...
export function downloadPNG() {
    ...
    if (svgElement) {
        try {
            ...
            notifications.success("PNG download started");
        } catch (e: any) {
            Logger.error("Error downloading PNG", e);
            notifications.error("Error downloading PNG: " + (e?.message || "Unknown error"));
        }
    } else {
        notifications.warning("No SVG to download. Please process an image first.");
    }
}
...
export function downloadSVG() {
    if ($("#svgContainer svg").length > 0) {
        try {
            ...
            notifications.success("SVG download started");
        } catch (e: any) {
            Logger.error("Error downloading SVG", e);
            notifications.error("Error downloading SVG: " + (e?.message || "Unknown error"));
        }
    } else {
        notifications.warning("No SVG to download. Please process an image first.");
    }
