            ...
            if (f != null) {
                ...
                for (let s: number = 0; s < segmentsPerFacet[f.id].length; s++) {
                    ...
                    if (segment != null && f.borderSegments[s] == null) {
                        ...
                        if (debug) {
                            Logger.debug("Setting facet " + f.id + " segment " + s + " to " + f.borderSegments[s]);
                        }
                        if (segment.neighbour !== -1) {
                                ...
                                for (let ns: number = 0; ns < neighbourSegments.length; ns++) {
                                    ...
                                    if (neighbourSegment != null && neighbourSegment.neighbour === f.id) {
                                        ...
                                        if (matchesStraight) {
                                            ...
                                            if (debug) {
                                                Logger.debug("Match found for facet " + f.id + " to neighbour " + neighbourFacet.id);
                                            }
                                            neighbourFacet!.borderSegments[ns] = new FacetBoundarySegment(segment, f.id, false);
                                            if (debug) {
                                                Logger.debug("Setting facet " + neighbourFacet!.id + " segment " + ns + " to " + neighbourFacet!.borderSegments[ns]);
                                            }
                                            ...
                                            if (debug) {
                                                Logger.debug("Reverse match found for facet " + f.id + " to neighbour " + neighbourFacet.id);
                                            }
                            ...
                            if (!matchFound && debug) {
                                ...
                                // cover all cases
                                Logger.error("No match found for segment of " + f.id + ": " +
